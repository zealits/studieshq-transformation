    ...
    for (const countryData of countryFieldsData) {
      try {
        ...
        if (!existing) {
          ...
          // Update existing record to ensure it has the latest field definitions
          await CountryBusinessFields.updateOne(
            { country: countryData.country },
            { $set: countryData }
          );
          console.log(`✓ Updated business fields for ${countryData.country} (ID: ${existing._id})`);
          created++;
        }
      } catch (error) {
